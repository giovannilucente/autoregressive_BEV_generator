...
def run_epoch(model, dataloader, criterion, optimizer=None):
    ...
    with torch.set_grad_enabled(is_train):
        for batch in tqdm(dataloader, desc="Training" if is_train else "Validating",  disable=not sys.stdout.isatty()):
...
def save_predictions(model, dataloader, output_dir, max_episodes=100):
    model.eval()
...
if __name__ == "__main__":
    ...
    # --- Datasets ---
    train_set = TrajectoryImageDatasetMultistep("../../AI_Datasets/_unzipped/WaymoMotionBEV", "train", num_steps, transform=transform)
    val_set = TrajectoryImageDatasetMultistep("../../AI_Datasets/_unzipped/WaymoMotionBEV", "validation", num_steps, transform=transform)
    test_set = TrajectoryImageDatasetMultistep("../../AI_Datasets/_unzipped/WaymoMotionBEV", "test", num_steps, transform=transform)
    ...
    for epoch in range(1, epochs + 1):
        ...
        test_output_dir = results_dir / "best_model_predictions"
        print(f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            save_predictions(model, val_loader, test_output_dir, max_episodes=100)
